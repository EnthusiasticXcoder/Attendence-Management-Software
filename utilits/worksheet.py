...
def entertheory(backfun,object,user,Excel,date,time,roll_list,TP):
    try:
        try:
            wb=load_workbook(f"workbooks\\{user}\\{Excel}")
        except:
        ...
        for col in range(10,n+11):
            enrollment.append(str(ws['B'+str(col)].value))
        
        enroll=[(str(i).zfill(3)).capitalize() for i in roll_list.split(",")]

        
        for col,roll in zip(range(10,n+10),enrollment):    
            for cheack in enroll:
                if roll.endswith(cheack):
                    object.add_frame(col,ws,"light green","P")
                    ws[char+str(col)] =  'P'
                    enroll.remove(cheack)
                    break
            else :
                ws[char+str(col)] =  'A'
                object.add_frame(col,ws,"red","A")
            ws['W'+str(col)]=f'=COUNTIF(D{str(col)}:V{str(col)},"P")'   #Theory
            ws['AF'+str(col)]=f'=COUNTIF(X{str(col)}:AE{str(col)},"P")' #Practicle
            ws['AG'+str(col)]=f'=SUM(W{str(col)},AF{str(col)})'

        ws[char+str(n+11)]=f'=COUNTIF({char}10:{char}{n-1},"P")'
        ws['AF'+str(n+11)]=f'=SUM(AF10:AF{n-1})'   # Practicle 
        ws['W'+str(n+11)]=f'=SUM(W10:W{n-1})'       # Theory
        ws['AG'+str(n+11)]=f'=SUM(W10:W{n-1})'      # Both
...
def select(backfun,object,user,sheet,date,time,roll_list,tp):
    ...
    if time == "" or time == " " or time=="Time" :
        ...
        return
    if tp :
        entertheory(backfun,object,user,sheet,date,time,roll_list,TP=tp)
    else:
        messagebox.showerror("Error","Invalid choice")
